	...
	*/
	...
	void FixedUpdate() {
		...
		if (grounded && doJump != 3) {
			...
			Vector3 targetSpeed = Vector3.Cross(groundNormal, newForward) * move.x * MovementSpeed + newForward * move.y * MovementSpeed;
			...
			if (doJump == 1) {
				...
				doJump = 2;
			} else if (!touchingDynamic && Mathf.Approximately(move.x + move.y, 0.0f) && doJump < 2)
			...
			// air accel
			if (!Mathf.Approximately(move.x + move.y, 0.0f)) {
				...
				movement = transform.TransformDirection(new Vector3(move.x * AirborneAccel * Time.deltaTime, 0.0f, move.y * AirborneAccel * Time.deltaTime));
